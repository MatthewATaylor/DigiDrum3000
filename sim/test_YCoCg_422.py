...
async def drive_pixel(dut, pixel, image_through, index):
    ...
    if index >= LATENCY:
        image_through.putpixel(
            ...
            pixel_to_rgb(int(dut.pixel_out.value)),
        )
    dut.pixel_in.value = (pixel[0] << 16) + (pixel[1] << 8) + pixel[2]
    dut.h_count_lsb.value = (index + LATENCY) % 2


@cocotb.test()
async def test_YCoCg_422(dut):
    ...
    # set all inputs to 0
    dut.h_count_lsb.value = 0
    dut.pixel_in.value = 0
    image_base = Image.new("RGB", (WIDTH, HEIGHT))
